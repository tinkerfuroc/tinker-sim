...
from challenge_grasp.utils.common import clip_and_scale_action
from challenge_grasp.utils.sapien_utils import get_entity_by_name, vectorize_pose
...
class PDEEPoseEulerController(PDEEPosController):
    config: "PDEEPoseControllerConfig"
    ...
    def compute_target_pose(self, prev_ee_pose_at_base, action):
        if self.config.use_delta:
            delta_pos, delta_rot = action[0:3], action[3:6]
            # delta_quat = Rotation.from_rotvec(delta_rot).as_quat()[[3, 0, 1, 2]]
            delta_quat = Rotation.from_euler('zyx', delta_rot).as_quat()[[3, 0, 1, 2]]
            delta_pose = sapien.Pose(delta_pos, delta_quat)
